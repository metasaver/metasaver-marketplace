...
# Strengthened pattern catches: git with flags (git -C path add), absolute paths (/usr/bin/git), command chaining (cd path && git add)
if echo "$COMMAND" | grep -qE '(rm\s+-rf\s+/[^a-zA-Z]|DROP\s+DATABASE|(^|[[:space:];|]|&&)(/?[\w/]*/)?(git)(\s+--?[\w-]+)*\s+(add|commit|push|pull|rebase|reset|merge|cherry-pick|revert|checkout|switch|restore|stash|am|apply))'; then
  cat >&2 << EOF
{
  "decision": "block",
